...
const state = { filter: '全部', island: '', selected: 'a', favorites: [] as string[], compare: ['a', 'b'], marks: [] as string[], question: '', draft: ['', '', '', ''], saved: false, crystals: 0, taskOpen: false, exploreRewarded: false };
try {
...
loadAppData().then(data => {
  ...
  function view(page: string) {
    ...
    if (page === 'home') return '<div class="topbar"><span>归一 UNIVERSE</span><b class="crystals">◇ ' + state.crystals + '</b></div>' + title('归一 UNIVERSE / 01', '一问，一座世界', '不急着下结论，先让不同的线索相遇。') + '<div class="bloomy"><div class="bloomy-art"><i></i><em></em></div><p>你好呀！一起去看更大的世界吧！</p></div><form id="question-form" class="question-card"><label for="question">你正在问什么？</label><textarea id="question" maxlength="160" required>' + escape(state.question) + '</textarea><button class="primary">进入这座世界 →</button></form><div class="section-row"><h2>四座岛，慢慢探索</h2><span class="muted">探索进度 · 占位</span></div><div id="map" class="map-wrap"></div><p class="center-note">中央归一水晶 · 世界地标</p><div class="island-grid">' + data.islands.map((i, index) => '<button class="' + (index > (state.saved ? 1 : 0) ? 'fogged' : '') + '" data-island="' + i.id + '" style="background:' + i.color + '"><b>' + i.name + '</b><small>' + i.note + '</small></button>').join('') + '</div><article class="mystery-box"><span class="tag">潘多拉任务 · 虚构</span><h2>打开被封锁的盒子，遇见意想不到的你</h2><p>一个未知岛入口的神秘练习。</p><button data-task>打开神秘盒子</button>' + (state.taskOpen ? '<div class="task-list">' + data.tasks.map(t => '<p>◇ ' + t + '<small>虚构任务示例</small></p>').join('') + '</div>' : '') + '</article>' + (state.saved ? '<button class="full" data-path>已解锁：可逆的小步尝试 →</button>' : '');
    if (page === 'explore') {
  ...
  function render() {
    ...
    if (page === 'home') { dispose = mountMap(root.querySelector('#map')!); if (state.saved) root.querySelector('#map')?.classList.add('map-unlocked'); }
    ...
    root.querySelector('#question-form')?.addEventListener('submit', e => { e.preventDefault(); if (!state.question.trim()) return; state.island = ''; if (!state.exploreRewarded) { state.exploreRewarded = true; state.crystals += 1; } persist(); go('explore'); });
    root.querySelector('[data-favorite]')?.addEventListener('click', () => { const added = !state.favorites.includes(state.selected); state.favorites = added ? [...state.favorites, state.selected] : state.favorites.filter(x => x !== state.selected); if (added) state.crystals += 1; persist(); render(); });
    ...
    root.querySelector('#shape-form')?.addEventListener('submit', e => { e.preventDefault(); if (state.draft.some(x => !x.trim())) return; if (!state.saved) state.crystals += 2; state.saved = true; const stored = persist(); render(); root.querySelector('#notice')!.textContent = stored ? '已保存到本机。' : '仅保留于本次会话；浏览器禁止本机存储。'; });
    root.querySelector('[data-path]')?.addEventListener('click', () => { state.island = ''; state.filter = '相关话题'; persist(); go('explore'); });
    root.querySelector('[data-task]')?.addEventListener('click', () => { state.taskOpen = !state.taskOpen; state.crystals += state.taskOpen ? 1 : 0; persist(); render(); });
  }
